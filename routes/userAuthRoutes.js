...
const FALLBACK_DUMMY_HASH = "$2a$12$Cj6UzMDM.H6dfI/f/IKcFei6dn9BpqyF3u6SX0plfDx2bUp7sXcXK";
...
router.get("/admin/all", verifyAdminToken, paginationValidation, async (req, res) => {
  try {
    ...
    res.json({ data: users, users, page, limit, total, totalPages: Math.ceil(total / limit) });
  } catch (error) {
...
router.post("/reset-password/:token", resetPasswordLimiter, resetPasswordValidation, async (req, res) => {
  try {
